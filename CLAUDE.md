...
The tech stack has been chosen and both apps are underway. `server-app` has the full harvest-tracking vertical slice implemented end-to-end — farms, auth (Firebase), fruits/harvesters/measurement-units catalogs, workdays (open/close), and the harvester-workday/harvest-entries offline-sync endpoints — following the DTO/service/controller conventions below, plus a `/health` check and rate limiting on auth. Known gaps: CI, e2e coverage beyond the scaffold test, and PaaS deployment aren't set up yet (see [arquitectura.md §3–5](docs/diagrams/arquitectura.md)). `ui-app` has its architecture scaffold in place — React Native Paper with a centralized theme, Drizzle/`expo-sqlite` local schema, Zustand stores, and Expo Router navigation with every screen present as a placeholder — see [docs/diagrams/ui-arquitectura.md](docs/diagrams/ui-arquitectura.md); no screen has real business logic yet.
