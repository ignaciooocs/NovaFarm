...
1. Probar en dispositivo físico lo recién implementado (ver arriba): que cerrar jornada funcione siempre ahora (era el bug que motivó esta ronda), editar/desactivar catálogo (sobre todo borrar el apodo de un harvester), `/team` (que un recorder no la vea ni pueda pegarle directo a `GET /users`), y el interruptor de catálogo (que un recorder deje de ver/vuelva a ver Frutas/Unidades/Cosechadores según lo prenda/apague el admin desde Ajustes).
2. Pulido, lo que queda: contenido real para Perfil (hoy placeholder, ya reachable desde el drawer) — Ajustes ya está.
