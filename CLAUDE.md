...
Navigation: `(app)/(drawer)/(tabs)` holds Home/History as bottom tabs; a Drawer wraps that plus Profile, Settings, and the catalogs and Team. `fruits`/`harvesters`/`measurementUnits` are cached locally (`lib/catalogSync.ts`, refreshed from Home and on reconnect) and support full editing + activate/deactivate against `server-app`'s `PATCH /:id` endpoints — adding an *existing* harvester to a roster works offline; registering a genuinely *new* one still needs connectivity (catalog creation itself has no offline path yet — a bigger, explicitly-deferred-but-prioritized gap needing a pending-local-row + idempotent-sync design, plus a local-id-to-server-id swap cascaded through any roster/entries recorded against that harvester before it synced). Whether a *recorder* (not just an admin) sees the catalog screens is a single per-farm toggle, `farms.recordersCanManageCatalog` (default `true`), not per-user permissions — cached client-side in `stores/useFarmSettingsStore.ts` (`lib/farmSettings.ts`, same fetch/reconnect pattern as `catalogSync.ts`) and editable by an admin from Settings. `POST /workdays` is idempotent via `clientEntryId` (retrying after a dropped response no longer risks a duplicate); `PATCH /workdays/:id/close` is too (closing an already-closed workday returns its frozen state instead of erroring). `/history` lists closed workdays live, resolving fruit names against the local cache. `/team` lists the farm's users — admin-only, unaffected by the catalog toggle above — enforced both by hiding it from a recorder's drawer and, server-side, by `RolesGuard` (`src/auth/guards/roles.guard.ts`, the project's first role check — pair with `FarmScopeGuard` via `@UseGuards(FarmScopeGuard, RolesGuard)` + `@Roles('admin')`); editing/deactivating a team member isn't built yet, by design (scoped down to viewing for now). Settings now shows real farm info + the catalog toggle; Profile is still a placeholder.
