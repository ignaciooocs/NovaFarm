...
The full harvest-tracking vertical slice is implemented end-to-end — farms (create, plus `GET`/`PATCH /farms/me` for the caller's own farm settings), auth (Firebase), fruits/harvesters/measurement-units catalogs (create/list/partial-update via `PATCH /:id`, the latter also doubling as activate/deactivate), a `users` controller (`GET /users` + `PATCH /users/:id/roles`), workdays (open/close), and the offline-sync endpoints (`POST /harvesters/sync`, `/harvester-workday/sync`, `/harvest-entries/sync` — all idempotent on a client-generated `clientEntryId`) — following the DTO/service/controller conventions below, plus a `/health` check and rate limiting on auth.
...
Identifiers, class names and DTO fields are English (see CLAUDE.md), but **code comments in `server-app` are written in Spanish**, and they explain *why* rather than restating the code — `fruits.service.ts` and `fruit.schema.ts` are the reference for the tone. Match that when editing existing files.
