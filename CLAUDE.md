...
The tech stack has been chosen and both apps are underway. `server-app` has the full harvest-tracking vertical slice implemented end-to-end — farms, auth (Firebase), fruits/harvesters/measurement-units catalogs (create/list/partial-update via `PATCH /:id`, the latter also doubling as activate/deactivate), a `users` controller (`GET /users`, admin-only — lists the caller farm's team), workdays (open/close), and the harvester-workday/harvest-entries offline-sync endpoints — following the DTO/service/controller conventions below, plus a `/health` check and rate limiting on auth. `AuthModule` and `UsersModule` import each other via `forwardRef()` (Auth needs Users for registration, Users now needs Auth's guards for its controller) — a legitimate, NestJS-sanctioned pattern for a genuine two-way dependency, not something to "fix" by removing. Known gaps: CI, e2e coverage beyond the scaffold test, and PaaS deployment aren't set up yet (see [arquitectura.md §3–5](docs/diagrams/arquitectura.md)).
...
Navigation: `(app)/(drawer)/(tabs)` holds Home/History as bottom tabs; a Drawer wraps that plus Profile, Settings, and (admin-only, hidden from a recorder's menu) the catalogs and Team. `fruits`/`harvesters`/`measurementUnits` are cached locally (`lib/catalogSync.ts`, refreshed from Home and on reconnect) and support full editing + activate/deactivate against `server-app`'s `PATCH /:id` endpoints — adding an *existing* harvester to a roster works offline; registering a genuinely *new* one still needs connectivity (catalog creation itself has no offline path yet — a bigger, explicitly-deferred-but-prioritized gap needing a pending-local-row + idempotent-sync design, plus a local-id-to-server-id swap cascaded through any roster/entries recorded against that harvester before it synced). `POST /workdays` is idempotent via `clientEntryId` (retrying after a dropped response no longer risks a duplicate). `/history` lists closed workdays live, resolving fruit names against the local cache. `/team` lists the farm's users — admin-only, enforced both by hiding it from a recorder's drawer and, server-side, by `RolesGuard` (`src/auth/guards/roles.guard.ts`, the project's first role check — pair with `FarmScopeGuard` via `@UseGuards(FarmScopeGuard, RolesGuard)` + `@Roles('admin')`); editing/deactivating a team member isn't built yet, by design (scoped down to viewing for now). Not built at all yet: real Settings/Profile content (screens exist, placeholders).
